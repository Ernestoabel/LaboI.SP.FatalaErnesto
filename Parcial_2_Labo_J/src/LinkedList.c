    ...
    if(this!=NULL && nodeIndex>=0 && nodeIndex<=ll_len(this)){
    	...
    	if(pNodeNuevo!=NULL){
			...
			if(nodeIndex==0){
				...
				if(aux!=NULL){
					pNodeNuevo->pNextNode=aux->pNextNode;
					aux->pNextNode=pNodeNuevo;
				}
			}
    	}
    	this->size++;
...
{
    int returnAux = -1,cantidad;
    cantidad=ll_len(this);
    if(this!=NULL){
    	addNode(this,cantidad,pElement);
    	returnAux=0;
    }
...
int ll_remove(LinkedList* this,int index)
{
    ...
    if(this!=NULL && index>=0 && index<ll_len(this)){
    	...
    	if(NodoEliminado!=NULL && NodeAnterior!=NULL){
    		NodeAnterior->pNextNode=NodoEliminado->pNextNode;
    	}else if(ll_len(this)==1){
    		this->pFirstNode=NULL;
    	}
    	NodoEliminado->pElement=NULL;
    	NodoEliminado->pNextNode=NULL;
    	this->size--;
    	returnAux=0;
    }
    return returnAux;
...
{
    int returnAux = -1,cantidad,i;
    cantidad=ll_len(this);
    Node* NodoEliminado=NULL;
    if(this!=NULL){
    	for(i=0;i<cantidad;i++){
    		NodoEliminado=getNode(this,i);
    		NodoEliminado->pNextNode=NULL;
    	}
    	this->pFirstNode=NULL;
    	this->size=0;
...
{
    int returnAux = -1,cantidad,i,bandera = 1;
    void* auxElement;
    cantidad = ll_len(this2);
    ...
    if(this!=NULL && (order==0 || order==1) && pFunc!=NULL){
    	for(int i=0;i<cantidad-1;i++){
    		for(int j=i+1;j<cantidad;j++){
    			if((pFunc(ll_get(this,i),ll_get(this,j))>0 && order==1) || (pFunc(ll_get(this,i),ll_get(this,j))<0 && order==0)){
    				pAux=ll_get(this,i);
